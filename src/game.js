...
const state = { ch:cloneChapter(), turn:1, phase:'ally', selected:null, mode:'move', moveCells:new Map(), targetCells:new Set(), over:false, logs:[] };
const ui = {};

async function loadMap(){
  const status=$('#mapStatus'), image=$('#mapImage');
  const texts=await Promise.all(Array.from({length:MAP.parts},async(_,i)=>{
    const r=await fetch(`${MAP.base}${i}.b64?v=13`,{cache:'no-store'}); if(!r.ok)throw new Error(`地图分片 ${i} HTTP ${r.status}`);
  ...
  status.textContent='地图 1120×800 · 28×20 逻辑棋盘已按 40×40 原生地形颗粒校准';
}
...
function setup(){
  ...
  buildGrid();bindCommands();const cao=state.ch.units.find(u=>u.id==='cao');$('#dialogPortrait').innerHTML=spriteSVG(cao);addLog('战斗开始：夺取桥头，击破张宝。');render();
}

function buildGrid(){
  ui.grid.innerHTML='';ui.grid.style.gridTemplateColumns=`repeat(${COLS},1fr)`;ui.grid.style.gridTemplateRows=`repeat(${ROWS},1fr)`;
  for(let y=0;y<ROWS;y++)for(let x=0;x<COLS;x++){const b=el('button','logic-cell');b.type='button';b.dataset.x=x;b.dataset.y=y;b.setAttribute('aria-label',`${x+1},${y+1}`);b.addEventListener('mouseenter',()=>showTerrain(x,y));b.addEventListener('focus',()=>showTerrain(x,y));b.addEventListener('click',()=>handleCell(x,y));ui.grid.appendChild(b);}
}
function bindCommands(){ui.attack.onclick=()=>setMode('attack');ui.skill.onclick=()=>setMode('skill');ui.wait.onclick=()=>finishUnit('待机');ui.cancel.onclick=()=>cancelAction();ui.end.onclick=()=>{if(state.phase==='ally'&&!state.over)enemyPhase();};$('#btnRestart').onclick=restart;}
...
function selectUnit(unit){if(state.over||state.phase!=='ally'||unit.side!=='ally'||unit.acted)return;state.selected=unit.id;state.mode='move';state.moveCells=unit.moved?new Map([[key(unit.x,unit.y),0]]):movement(unit);state.targetCells.clear();render();}
function handleCell(x,y){
  if(state.over||state.phase!=='ally')return;const hit=unitAt(x,y),u=selected();if(!u){if(hit?.side==='ally')selectUnit(hit);return;}
...
function setMode(mode){const u=selected();if(!u||u.acted||state.phase!=='ally')return;if(mode==='skill'){if(!u.skill){addLog(`${u.name} 没有可用技能。`);return;}if(u.skill.cd>0){addLog(`${u.skill.name} 还需 ${u.skill.cd} 回合冷却。`);return;}if(u.mp<u.skill.mp){addLog('MP不足。');return;}state.targetCells=targetSet(u,1,u.skill.range);}else state.targetCells=targetSet(u,u.minR,u.maxR);state.mode=mode;render();}
function cancelAction(){const u=selected();if(!u)return;state.mode='move';state.targetCells.clear();state.moveCells=u.moved?new Map([[key(u.x,u.y),0]]):movement(u);render();}
function matchup(a,d){const table={cavalry:{infantry:1.15,archer:0.9,spear:0.82},infantry:{archer:1.12,spear:1.08},archer:{cavalry:1.18},spear:{cavalry:1.22}};return table[a.cls]?.[d.cls]||1;}
...
function resolveAttack(a,d,isSkill){const sk=isSkill?a.skill:null,dmg=damage(a,d,sk?.power||1);d.hp=Math.max(0,d.hp-dmg);if(sk){a.mp-=sk.mp;sk.cd=sk.cdMax;addLog(`${a.name} 施展「${sk.name}」，对 ${d.name} 造成 ${dmg} 点伤害。`);}else addLog(`${a.name} 攻击 ${d.name}，造成 ${dmg} 点伤害。`);fx(d.x,d.y,isSkill?'skill':'hit',dmg);if(d.hp<=0){d.dead=true;addLog(`${d.name} 被击破！`);}a.acted=true;state.selected=null;state.mode='move';state.targetCells.clear();state.moveCells.clear();checkEnd();render();if(!state.over&&living('ally').every(x=>x.acted))setTimeout(enemyPhase,450);}
function finishUnit(reason){const u=selected();if(!u||u.acted)return;u.acted=true;addLog(`${u.name} ${reason}。`);state.selected=null;state.mode='move';state.targetCells.clear();state.moveCells.clear();render();if(living('ally').every(x=>x.acted))setTimeout(enemyPhase,350);}
...
function chooseTarget(e,targets){return[...targets].sort((a,b)=>{const da=manhattan(e,a),db=manhattan(e,b);if(da!==db)return da-db;return a.hp-b.hp;})[0];}
function moveEnemy(e,target){
  const dist=movement(e),ai=e.ai;let choices=[];for(const[k,cost]of dist){const[x,y]=k.split(',').map(Number);if(x===e.x&&y===e.y)continue;const d=Math.abs(x-target.x)+Math.abs(y-target.y);let score=d*10+cost;if(ai?.role==='ranged'){const ideal=Math.max(e.minR,Math.min(e.maxR,5));score=Math.abs(d-ideal)*14+cost;if(d<e.minR)score+=30;}if(ai?.anchorX!=null){const anchor=Math.abs(x-ai.anchorX)+Math.abs(y-ai.anchorY);if(anchor>(ai.leash||99))score+=50+(anchor-(ai.leash||99))*10;}choices.push({x,y,score});}choices.sort((a,b)=>a.score-b.score);if(choices[0]){e.x=choices[0].x;e.y=choices[0].y;render();}}
...
// 小型像素兵将：回到此前参考图的曹操传式轮廓，不再使用光滑卡通 SVG。
function spriteSVG(u){
  const ally=u.side==='ally',blue='#214f86',blue2='#132f55',red='#8f2e25',red2='#571d18',main=ally?blue:red,dark=ally?blue2:red2,gold='#d0a84e',skin='#c89561',ink='#17130f';
  const px=(x,y,w,h,c)=>`<rect x="${x}" y="${y}" width="${w}" height="${h}" fill="${c}"/>`;
  const spear=`${px(34,5,2,37,'#b7aa84')}${px(32,4,6,4,'#d9d1b7')}`;
  const bow=`<path d="M34 10 Q44 24 34 39" fill="none" stroke="#b98c48" stroke-width="3"/><path d="M34 10 L34 39" stroke="#d9d0b2" stroke-width="1"/>`;
  const sword=`<path d="M33 18 L40 7" stroke="#ddd8c9" stroke-width="3"/><path d="M31 20 L36 22" stroke="#a87935" stroke-width="2"/>`;
  const weapon=u.cls==='archer'?bow:u.cls==='spear'?spear:sword;
  const horse=u.cls==='cavalry'?`<g>${px(8,36,25,8,'#513721')}${px(11,43,4,10,'#2b2118')}${px(27,43,4,10,'#2b2118')}${px(29,31,8,8,'#5c4026')}${px(35,28,4,7,'#3a291c')}${px(7,37,4,3,'#2b2118')}</g>`:'';
  const plume=(u.id==='cao'||u.id==='dun'||u.cls==='boss')?`${px(21,2,3,7,'#b93b2e')}${px(24,1,3,4,'#d4553d')}`:'';
  const boss=u.cls==='boss',robe=boss?'#6d356f':main;
  return `<svg viewBox="0 0 48 58" aria-hidden="true" shape-rendering="crispEdges">${horse}<ellipse cx="24" cy="54" rx="15" ry="3" fill="#000" opacity=".35"/>${plume}${px(17,9,14,5,ink)}${px(15,13,18,5,gold)}${px(18,17,12,9,skin)}${px(19,19,2,2,ink)}${px(27,19,2,2,ink)}${px(14,25,20,16,robe)}${px(17,28,14,10,dark)}${px(14,27,4,14,gold)}${px(30,27,4,14,gold)}${px(20,31,8,5,gold)}${px(17,41,6,10,dark)}${px(27,41,6,10,dark)}${weapon}</svg>`;
}
...
function render(){ui.turn.textContent=`第 ${state.turn} 回合`;ui.phase.textContent=state.phase==='ally'?'我军阶段':state.phase==='enemy'?'敌军阶段':'战斗结束';ui.goal.textContent=state.ch.objective;ui.score.textContent=`${living('ally').length} : ${living('enemy').length}`;renderRange();renderUnits();const u=selected();if(u)showUnit(u);else if(state.phase==='ally')ui.info.innerHTML='<div class="panel-title">武将 <span>未选择</span></div><div class="empty-info">点击我军武将查看信息并行动。</div>';const enabled=!!u&&state.phase==='ally'&&!u.acted;ui.attack.disabled=!enabled;ui.skill.disabled=!enabled||!u?.skill;ui.wait.disabled=!enabled;ui.cancel.disabled=!enabled;ui.end.disabled=state.phase!=='ally'||state.over;ui.command.textContent=state.over?'战斗结束。':!u?'选择我军武将开始行动。':state.mode==='move'?'蓝色区域可移动；注意树林、山岩与河流会改变路线。':state.mode==='attack'?'选择红色范围内的敌军。':'选择紫色范围内的敌军施放技能。';}
