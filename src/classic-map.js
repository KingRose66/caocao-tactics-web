...
function paint(){if(!F)return;layout();const {x,w,h}=canvas();x.clearRect(0,0,w,h);const first=(document.querySelector('#title')?.textContent||'').includes('颍川');if(first&&fullMap.complete&&fullMap.naturalWidth){x.imageSmoothingEnabled=true;x.drawImage(fullMap,0,0,w,h)}else{x.imageSmoothingEnabled=false;paintFallback(x,w,h)}}
fullMap.onload=paint;atlas.onload=paint;new MutationObserver(paint).observe(F,{childList:true,subtree:true});addEventListener('resize',paint);setTimeout(paint,30);setTimeout(paint,180);setTimeout(paint,500);
